...
@section('content')
    ...
    <section id="show">
        ...
        <div class="movie">
            ...
            <div class="movie__bg" style="background: linear-gradient(rgba(0,0,0, 0.6), rgba(0,0,0, 0.6)), url({{ asset('images/'.$movie->image) }}) center/cover no-repeat;"></div>
            ...
            <div class="container">
                ...
                <div class="row">
                    ...
                    <iframe style="margin-top:14px" width="700" height="394 " src="{{ $movie->link }}" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
        ...
        <div class="container">
            ...
            <div class="movies owl-carousel owl-theme">
                ...
                @foreach ($related_movies as $related_movie)
                    ...
                    <div class="movie p-0">
                        <img src="{{ asset('images/'.$related_movie->poster) }}" class="img-fluid" alt="">
...
@push('scripts')
    ...
    <script>
        var file =
            "[Auto]{{ Storage::url('app/movies/' . $movie->id . '/' . $movie->id . '.m3u8') }}," +
            "[360]{{ Storage::url('app/movies/' . $movie->id . '/' . $movie->id . '_100.m3u8') }}," +
            "[480]{{ Storage::url('app/movies/' . $movie->id . '/' . $movie->id . '_250.m3u8') }}," +
            "[720]{{ Storage::url('app/movies/' . $movie->id . '/' . $movie->id . '_500.m3u8') }}";
